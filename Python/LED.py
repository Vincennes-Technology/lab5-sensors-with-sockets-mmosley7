...
COLOR = [0xFF0000, 0x00FF00, 0x0000FF, 0xFFFF00, 0xFF00FF, 0x00FFFF, 0x0F0F0F]
...
def main():
    ...
    while True:
        for color in COLOR:
            ...
            data = "'LOWSGLOW',%d, 'color_msg: R_val = %s, G_val = %s, B_val = %s"\
             % (n,colorvalues[0],colorvalues[1],colorvalues[2])
            sock.sendall(data)
            print " Sent:", data
            sock.close()
            n += 1
...
if __name__ == '__main__':
    ...
    try:
        main()
        # When 'trl+C' is pressed, the child program
